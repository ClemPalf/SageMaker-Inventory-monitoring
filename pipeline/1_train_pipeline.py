...
def train(model, train_loader, val_loader, criterion, optimizer, device):
    ...
    for epoch in range(epochs):
        ...
        current_val_loss = test(model, val_loader, criterion, "val", device)    # Pass the SMDebug hook to the test function
...
def main(args):
    ...
    model=train(model, train_loader, val_loader, criterion, optimizer, device)  # Pass the SMDebug hook to the train function
    
    '''
    Test the model to see its accuracy
    '''
    #logger.info("Starting model evaluation...")
    test(model, test_loader, criterion, "test", device) # Pass the SMDebug hook to the test function
...
if __name__=='__main__':
    ...
    parser.add_argument('--data', type=str, default=os.environ['SM_CHANNEL_TRAINING']) # When training with pipelines, the env variable name slighty change (normally its SM_CHANNEL_TRAIN)
